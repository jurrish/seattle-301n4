...
(function(module) {
  ...
  Article.numWordsAll = function() {
    return Article.all.map(function(article) {
      ...
      return article.body.match(/\s+/g).length;
      //return an array with every item in the string separated by spaces
  ...
  Article.allAuthors = function() {
    ...
    .reduce(function(acc, curr) {
      if(acc.indexOf(curr) === -1){//this is the check to see if the accumulator array contains the current value of article.author, and if it doesnt (-1) then it pushes to the acc array
        acc.push(curr);
      };
      return acc;
    }, []);
    ...
    return Article.allAuthors().map(function(author) {
      return {
        ...
        numWords: Article.all.filter(function(article) {
          ...
          return articleObject.body.match(/\s+/g).length;
        })
        .reduce(function(acc, curr) {
          return acc + curr;
        })
